...
> 迁移状态（2026-09-02）：本文保留为 Pi Agent + DeepSeek 时期的架构历史基线；其运行时与账户架构已由 `docs/28-Codex-App-Server架构迁移技术适配声明.md` 和 `docs/29-Codex-App-Server迁移第一阶段开发文档.md` 取代。
>
> 文档状态：G5 已确认
>
...
>
> 当前边界：最终产品方案、七层架构和生产路线已经确认；当前只生成 V0.2-B 最小 Harness 的 G6 阶段文档，不开放写入型生产工具，不修改生产代码
...
- 复用当前产品流程开始时已锁定的三份手册快照，并加载最小必要项目知识；
...
| ManualAuthority | 每个新产品流程开始时校验、完整读取并锁定手册快照；流程内复用，结束后释放 | 文件定位、SHA、稳定顺序、受保护上下文、生命周期和加载记录 | 缺失或哈希不符立即阻塞；流程结束后不再读取 |
...
用户输入一句需求 / PRD / 文档
→ WebUI 创建产品项目并登记原始输入
→ ManualAuthority 校验、完整读取并锁定该新产品流程的三份手册快照
...
→ FactoryHarness 复用 ManualAuthority 已锁定的快照，并加载已确认产物和当前工作区
...
权威顺序：
1. 当前产品流程开始时已校验、完整读取并锁定的三份原始手册全文快照；
2. 已确认 PRD；
3. 已确认上游产物；
...
1. ManualAuthority 在新产品流程开始时校验、完整读取并锁定三份原始手册；当前工位只复用已锁定快照，流程完成或终止后释放且不再读取；
...
当前结果：G1–G5 已确认，当前进入阶段 B 的 G6 阶段文档确认。
...
- [x] 最终产品是“Web 控制平面 + 确定性 Workflow + 单 Factory Agent Harness”的本地优先产品工厂；
- [x] 七层架构、控制平面和执行平面的职责没有理解偏差；
- [x] G1–G9、生产任务、工具、证据和发布路线符合逐步确认要求；
- [x] 普通操作自动执行，删除、费用、外部发送、Git push 和发布继续需要确认；
- [x] 第一版继续使用 Node.js、TypeScript、Next.js、SQLite、Pi Agent 和 DeepSeek，不新增外部平台；
- [x] 先建设最小可执行 Harness，再建设恢复与 Goal Gate，然后小游戏试产和非游戏验证；
- [x] Subagent、Memory、Teams、Cron、MCP、多用户和公开部署继续不进入当前范围；
- [x] 当前代码缺口仍需 G6 后真实开发和双层验收，不能把本文档当成已经实现。

本清单已由产品负责人确认，G5 正式通过。下一步只确认 G6 的阶段开发文档、生产单和完成目标；G6 通过前不修改生产代码。
