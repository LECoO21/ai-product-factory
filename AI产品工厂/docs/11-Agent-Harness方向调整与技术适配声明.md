...
> 状态：方向、流程、正式 PRD 和 G2–G4 已确认。当前等待 `docs/16-AI产品工厂-七层架构与产品生产蓝图.md` 的 G5 确认；G6 之前不实施新方向的生产代码。
>
...
1. 每次生产运行开始先校验三份原文 SHA256；校验失败立即阻塞。
2. 按当前阶段完整读取对应原始手册；跨阶段任务完整读取三份。
3. 原文按稳定顺序进入受保护上下文，内容不改写，手册消息不参与历史摘要。
4. 运行事件记录手册名称、哈希、加载时间和适用阶段，但不记录或公开原文。
5. 派生 Skill、索引或摘要只能用于导航，不能作为合规依据；执行前必须回到原文。
6. 固定手册前缀优先使用模型上下文缓存，以减少延迟；缓存只改变传输成本，不改变输入内容。
...
- 三份手册：原文受保护、完整加载、哈希留证、永不以摘要替代；
- 第一台试产：小游戏；第二台使用非游戏产品验证通用性。

本方向、流程、正式 PRD 和 G2–G4 已经确认。下一道闸门是 G5 产品方案确认；随后仍需完成 G6，G6 通过后才进入 Harness 代码实施。
