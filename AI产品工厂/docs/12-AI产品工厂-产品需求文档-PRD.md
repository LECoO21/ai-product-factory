...
> 文档状态：G1 已确认；2026-09-02 Codex App Server 迁移要求已合并
>
> 产品阶段：单用户本地优先 MVP
>
...
Factory Agent 的唯一运行引擎是 Codex App Server。用户必须先登录自己的 ChatGPT/OpenAI 账户；图片、音频和 3D 素材由 Codex 编排独立工位和真实工具生产。
...
10. 使用前通过 Codex App Server 完成 ChatGPT/OpenAI 账户登录，工厂不接收模型 API Key。
11. 按需生产图片、音频和 3D 素材；缺少真实工具时显示“能力未配置”并阻塞对应工位。
...
- 未登录 OpenAI 账户时无法进入产品生产页或启动 Codex Turn；
- 工厂不在浏览器、SQLite、日志或仓库中保存 OpenAI Token；
- 媒体工位的成功结果必须指向真实 Artifact，未配置能力不得伪造成功；
...
- 产品效果验收、发布候选和发布确认；
- 产品历史、产物、失败原因和恢复入口。
- Codex App Server 账户登录、退出、Thread/Turn 绑定和中断/引导。
- 图片、音频、3D 素材工位、能力状态和 Artifact 登记。
...
- 标准 PRD 已确认；
- 本次产品流程已完整读取并锁定三份原始手册的 SHA256 快照；
- 现有代码、环境和约束已经检查。

#### 工厂动作

Factory Agent 复用本次产品流程开始时已锁定的三份完整原文快照，以其构成的生命周期为最高工程规则，输出：
...
- 由 Codex App Server 维持 Thread/Turn，所有动态工具请求仍经 ToolGateway 裁决。
- 蓝图触发多媒体时进入图片、音频或 3D 工位；对应工具未配置时阻塞并保留上游成果。
...
- G8 通过后，复用已锁定的完整上线部署手册快照生成部署方案；
- 创建计费资源、配置正式凭证和执行线上部署必须单独确认；
- 部署方案进入 G9，确认后生成发布候选和手工发布清单；
- 产品工厂在本版本不登录云平台、不创建资源、不执行 Git push、不部署或切换流量。
...
| G9 发布准备确认 | 部署、费用、凭证和回滚方案 | 生成发布候选和手工发布清单，不实际发布 |
...
- 未登录时只显示登录页和必要认证 API；
- 登录、取消登录和退出由 Worker 通过 Codex App Server 执行；
- Web 通过 SQLite 命令队列等待结果，不直接启动 App Server；
- 只保存脱敏账户状态，不保存访问令牌。
...
- 每个产品流程开始时校验并按固定顺序完整读取三份原始手册；
- 锁定本次流程的完整原文快照与哈希，流程内所有工位复用同一快照；
- 流程完成或终止后不再读取；新产品流程重新校验和锁定；
- 校验失败或原文缺失时停止生产；
...
- Codex App Server 是唯一生产 Agent 运行时；
- 一个产品项目绑定一个可恢复 Codex Thread，一个生产批次绑定一个 Turn；
...
- 图片、音频和 3D 是独立工位，拥有各自的输入、能力状态、产物和验收条件；
- Codex 负责规划并调用已配置 Tool、MCP 或 CLI Adapter；
- 图片能力仅在检测到真实工具时可用；音频、3D 未配置时明确阻塞；
- 占位文件、示例文本或模型自述不能作为素材成功证据。
...
- 未登录：只显示 ChatGPT/OpenAI 登录、进度、取消和重试；
- 左侧：产品项目和历史记录；
- 中间：当前产品的对话、产物和下一步；
- 顶部或内容区：当前所处重大阶段；
...
未安装 Codex CLI、App Server 启动失败或账户未登录时，系统进入可恢复阻塞态，显示实际原因，不回退到其他模型或伪造结果。

### 12.11 素材能力未配置

某个产品蓝图需要图片、音频或 3D，但当前没有真实工具时，只阻塞对应工位并保留上游产物，不用占位素材冒充完成。

## 13. 非功能要求

### 13.1 安全

- OpenAI 登录凭据只由 Codex 自身的凭据存储管理，产品工厂不接收、复制或持久化；
- 访问令牌和部署凭据不能进入浏览器、日志、SQLite 事件、模型消息或产物；
- 文件路径必须经过工作区边界校验；
- 高风险动作默认拒绝或进入人工闸门；
...
- 确定性业务逻辑可离线自动测试；
- App Server 协议具有伪造运输测试和已登录账户的真实 Turn 冒烟；
...
- Codex App Server 在锁定的长手册上下文下的首字时间、缓存命中和用量；
...
- [x] 每个新产品流程开始时完整读取并锁定三手册快照，流程内所有工位、返工、重试和恢复复用；完成或终止后释放且不再读取；下一个新产品重新读取。
- [x] Factory Agent 只使用 Codex App Server，用户必须登录自己的 ChatGPT/OpenAI 账户。
- [x] 图片、音频、3D 使用独立工位和真实工具，未配置时不伪造产物。
- [x] 上线阶段只生成流程、发布候选和手工清单，不执行实际发布。

本清单已由产品负责人确认。G1–G9 业务流程继续有效；Agent 运行时、OpenAI 登录和素材工位以 2026-09-02 Codex App Server 迁移要求为准。
